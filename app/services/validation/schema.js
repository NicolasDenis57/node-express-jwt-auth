...
const userSchema = Joi.object({
  ...
  email: Joi.string().email().messages({
    ...
    createUser: (userSchema) => userSchema.required(),
    update: (userSchema) => userSchema.optional()
  }),
  password: Joi.string()
  ...
  firstname: Joi.string().pattern(new RegExp('^[a-zA-Z]+$')).min(1).messages({
  ...
  // cette RegExp permet de vérifier que le prénom et le nom ne contiennent que des lettres.
  lastname: Joi.string().pattern(new RegExp('^[a-zA-Z]+$')).min(1).messages({
